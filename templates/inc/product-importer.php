...
function spek_render_product_importer_page(): void
{
    ...
    <div class="wrap">
        ...
        <p>
            <?php esc_html_e('Ο importer δημιουργεί ή ενημερώνει προϊόντα με βάση το SKU, δημιουργεί αυτόματα κατηγορίες και εισάγει τις εικόνες στη Media Library.', 'spek-theme'); ?>
        </p>
        ...
        <div class="notice notice-info inline">
            <p>
                ...
                <?php esc_html_e('Υποστηρίζονται και οι παλιές στήλες. Οι γραμμές με import_action διαφορετικό από import ή review_status διαφορετικό από ready παραλείπονται. Δημιουργείται ή ενημερώνεται ένα canonical προϊόν ανά SKU. Τα παρεχόμενα αγγλικά πεδία αποθηκεύονται απευθείας στο SPEK i18n backend και δεν δημιουργείται δεύτερο product post. Τα κενά τεχνικά πεδία δεν διαγράφουν υπάρχουσες τιμές.', 'spek-theme'); ?>
            </p>
        </div>
        ...
        <form id="spek-product-import-form" enctype="multipart/form-data">
            <table class="form-table" role="presentation">
                ...
                <tr>
                    <th scope="row"><?php esc_html_e('Λειτουργία', 'spek-theme'); ?></th>
                    <td>
                        <label>
    ...
    (function () {
        ...
        function renderStats(stats, done) {
            ...
            progressText.textContent = prefix + ': ' +
                ...
                <?php echo wp_json_encode(__(' · Ενημερώσεις ', 'spek-theme')); ?> + stats.updated +
                <?php echo wp_json_encode(__(' · Παραλείφθηκαν ', 'spek-theme')); ?> + stats.skipped +
            ...
            let html = '<div class="notice ' + (stats.error_count ? 'notice-warning' : 'notice-success') + ' inline"><p><strong>' +
                ...
                '</strong><br>' + escapeHtml(<?php echo wp_json_encode(__('Γραμμές: ', 'spek-theme')); ?>) + '' + stats.rows +
                <?php echo wp_json_encode(__(' · Νέα: ', 'spek-theme')); ?> + stats.created +
                <?php echo wp_json_encode(__(' · Ενημερώσεις: ', 'spek-theme')); ?> + stats.updated +
                <?php echo wp_json_encode(__(' · Παραλείφθηκαν: ', 'spek-theme')); ?> + stats.skipped +
            ...
            if (stats.warning_count) {
                html += '<p>' + <?php echo wp_json_encode(__('Προειδοποιήσεις:', 'spek-theme')); ?> + ' ' + stats.warning_count + '</p><ul>';
...
function spek_ajax_product_import_batch(): void
{
    ...
    // Dry run is cheap. Real import stays at one product/request because image
    // downloads and WordPress image processing can otherwise exceed nginx timeouts.
    $batch_size = $dry_run ? 40 : 1;
