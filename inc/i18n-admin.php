...
function spek_i18n_render_post_metabox(WP_Post $post): void {
    ...
    $defaults = function_exists('spek_i18n_default_strings_en') ? spek_i18n_default_strings_en() : [];
    $default_title = isset($defaults[$post->post_title]) ? (string) $defaults[$post->post_title] : '';
    $default_excerpt = ($post->post_excerpt !== '' && isset($defaults[$post->post_excerpt])) ? (string) $defaults[$post->post_excerpt] : '';
    ?>
    ...
    <table class="form-table" role="presentation">
        <tr>
            ...
            <td>
                <input class="widefat" type="text" id="spek_i18n_title_en" name="spek_i18n_title_en" value="<?php echo esc_attr($title); ?>" placeholder="<?php echo esc_attr($default_title); ?>">
                <?php if ($title === '' && $default_title !== '') : ?><p class="description"><?php echo esc_html(sprintf(__('Προεπιλεγμένη μετάφραση theme: %s', 'spek-theme'), $default_title)); ?></p><?php endif; ?>
            </td>
        </tr>
        <tr>
            <th><label for="spek_i18n_excerpt_en"><?php esc_html_e('Σύντομη περιγραφή / Excerpt (English)', 'spek-theme'); ?></label></th>
            <td>
                <textarea class="widefat" rows="3" id="spek_i18n_excerpt_en" name="spek_i18n_excerpt_en" placeholder="<?php echo esc_attr($default_excerpt); ?>"><?php echo esc_textarea($excerpt); ?></textarea>
                <?php if ($excerpt === '' && $default_excerpt !== '') : ?><p class="description"><?php echo esc_html(sprintf(__('Προεπιλεγμένη μετάφραση theme: %s', 'spek-theme'), $default_excerpt)); ?></p><?php endif; ?>
            </td>
        </tr>
        <tr>
